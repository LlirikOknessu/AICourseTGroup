...
if __name__ == '__main__':
    ...
    BATCH_SIZE = 64 # Определяет количество образцов, которые одновременно проходят через сеть.
    BUFFER_SIZE = 128 # Определяет, сколько примеров загружается в память и перемешивается вместе перед выбором пакета.
    ...
    # Обучение модели
    model.fit(X_train, y_train, batch_size=BATCH_SIZE, epochs=epochs, validation_data=(X_val, y_val), callbacks=[tensorboard_callback, checkpoint_callback])
