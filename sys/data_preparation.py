...
if __name__ == '__main__':
    ...
    # Read and concatenate the two datasets
    student_mat = pd.read_csv(input_dir / 'student-mat.csv')
    student_por = pd.read_csv(input_dir / 'student-por.csv')
    students = pd.concat([student_mat, student_por], ignore_index=True)

    cleaned_data = clean_data(df=students)

    # 'G3' is the target column
    X, y = cleaned_data.drop("G3", axis=1), cleaned_data['G3']
