...
def build_gru_model(input_shape, params):
    model = Sequential()
    model.add(GRU(params['gru_units'], input_shape=input_shape, return_sequences=False))
    model.add(Dense(1))
...
if __name__ == '__main__':
    ...
    # Reshape data to fit GRU input
    X_train_scaled = X_train_scaled.reshape((X_train_scaled.shape[0], 1, X_train_scaled.shape[1]))
    X_val_scaled = X_val_scaled.reshape((X_val_scaled.shape[0], 1, X_val_scaled.shape[1]))

    model = build_gru_model((X_train_scaled.shape[1], X_train_scaled.shape[2]), params)
